...
function CadastroAgendamentos() {
  ...
  function inicializar() {
    if (idParam == null) {
      ...
      const dataHora = new Date(dadosAgendamento.dataHora);
      setData(dataHora.toLocaleDateString());
      setHora(dataHora.toLocaleTimeString());
    }
  }

  async function salvar() {
    let dataHora = `${data}T${hora}`; // Concatenando a data e hora no formato correto
              ...
              <FormGroup label="Médico: *" htmlFor="inputMedico">
                ...
                >
                  ...
                  {medicos
                    .filter(
                      (funcionario) =>
                        funcionario.tipoFuncionario === "Médico"
                    ) 
                    .map((medico) => (
